...
namespace CFT.CSOMS.BLL.CheckModoule
{
    ...
    public class Check_Class
    {
        ...
        public static bool CreateCheck(string strUserID, string strMainID, string strCheckType, string strMemo, string strLevelValue, Param[] myParams, string user, string ip)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("你已发起一个相同审批类型的审批！");
                }
                ...
                if (strCheckType == "LCTBalanceRedeem")
                {
                    ...
                    {
                        throw new Exception("该财付通账号已经有待审批的理财通余额强赎！");
                    }
                }
                ...
                else
                {
                    if (iLevel <= 0)
                    {
                        throw new Exception("你发起的审批无人处理，请核对审批类型和配置是否正确！");
                    }
        ...
        private static void GetNextCheckLevel(string strMainID, MySqlAccess da)
        {
            ...
            {
                throw new Exception("查找不到指定的审批记录！");
            }
        ...
        /// </summary>
        /// <param name="strCheckID"></param>
        private static void ValidCheck(string strCheckID, string user, string ip, MySqlAccess da)
        {
            //第一步，如果审批是不同意，审批全部结束。   
            ...
            {
                if (!FinishCheck.ExecuteCheck(strCheckID, user, ip, da))
                {
                    //这时候要引发异常来导致外面rollback
                    throw new Exception("执行审批任务时出现了错误！");
                }
            }
        }
    }
