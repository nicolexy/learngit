...
namespace TENCENT.OSS.CFT.KF.KF_Web.BaseAccount
{
    ...
    /// GatherLog 的摘要说明。
    /// </summary>
    public partial class GatherLog : TENCENT.OSS.CFT.KF.KF_Web.PageBase
    {
        protected DataTable dataTable1;
        protected DataColumn dataColumn1;
        protected DataSet DS_Gather;
        protected DataGrid DG_TBankGather;
        ...
        private void BindData(int pageIndex)
        {
            ...
            int pdaycount = int.Parse(PublicRes.GetZWDicValue("ShouKuanDayCount"));//收款向前查询天数  
            ...
            if (Request.QueryString["type"].ToString() == "QQID")
            {
                ...
                this.DS_Gather = new TradeService().GetTCBankRollList(selectStr, 0, beginTime, endTime, true, istr, imax);
            ...
            else if (Request.QueryString["type"].ToString() == "ListID")
            {
                ...
                //当取数据条数发生变化时，分页组件和grid显示条数相应的pagesize属性都要跟着更改。 
                this.DS_Gather = new TradeService().GetTCBankRollList(selectStr, 1, beginTime, endTime, true, istr, imax);
            }
