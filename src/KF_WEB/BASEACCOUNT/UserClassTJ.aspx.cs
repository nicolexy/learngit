...
namespace TENCENT.OSS.CFT.KF.KF_Web.BaseAccount
{
	...
	public partial class UserClassTJ : System.Web.UI.Page
	{
		...
		private void ValidateDate()
		{
			...
			if(begindate.AddDays(3)< enddate)
			{
				throw new Exception("日期间隔超过三天！");
			}
		...
		{
			string stmp = "UserClassQuery.aspx?state=" + state + "&begin=" + DateTime.Parse(pagebegindate).ToString("yyyy-MM-dd") 
				+ "&end=" + DateTime.Parse(pageenddate).ToString("yyyy-MM-dd") + "&user=" + user;
			return stmp;
		}

		private void BindData()
		{/* 页面已被废除 begin
			DateTime begindate = DateTime.Parse(ViewState["begindate"].ToString());
			DateTime enddate = DateTime.Parse(ViewState["enddate"].ToString());
			...
			string msg = "";
			DataSet ds = qs.GetUserClassPickTJ(begindate,enddate,out msg);
			...
			//再查询一下各统计信息.
			string[] countlist = qs.GetUserClassSum(begindate,enddate);
		...
		protected void btnTJ_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				string content = struserdata + "执行了[实名认证统计]操作,操作对象[" + " "
					+ "]时间:" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

				Common.AllUserRight.UpdateSession(strszkey,ioperid,PublicRes.GROUPID,iserviceid,struserdata,content);

				string log = SensitivePowerOperaLib.MakeLog("get",struserdata,"[实名认证统计]","");
