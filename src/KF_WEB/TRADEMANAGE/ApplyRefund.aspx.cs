...
namespace TENCENT.OSS.CFT.KF.KF_Web.TradeManage
{
	...
	/// ApplyRefund 的摘要说明。
	/// </summary>
	public partial class ApplyRefund : TENCENT.OSS.CFT.KF.KF_Web.PageBase
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				...
				if(!IsPostBack)
				{
					this.btnApply.Attributes["onClick"] = "if(!confirm('确定要申请开通退款吗？')) return false;"; 
				}
			}
			catch
			{
		...
		protected void btnApply_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				string content = struserdata + "执行了[申请开通退款]操作,操作对象[" + this.txtFspid.Text.Trim()
					+ "]时间:" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

				Common.AllUserRight.UpdateSession(strszkey,ioperid,PublicRes.GROUPID,iserviceid,struserdata,content);

				string log = SensitivePowerOperaLib.MakeLog("get",struserdata,"[申请开通退款]",this.txtFspid.Text.Trim(),
					Session["uid"].ToString(),this.txtReason.Text.Trim());
			...
			try
			{
                ...
                {
                    throw new Exception("开通退款错误：" + spoa_ret);
                }
			}
